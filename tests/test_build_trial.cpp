...
// additional stuff
  ...
  //   volTot += dV[i][0]*(dp[j][0]-dp[0][0])+dV[i][1]*(dp[j][1]-dp[0][1])+dV[i][2]*(dp[j][2]-dp[0][2]);
  // }
  // printf("dV: %g\n", volTot);
  ...
  //   printf("%u: ", i);
  //   real_t dASq = (areasNew[i][0]*areasNew[i][0] + areasNew[i][1]*areasNew[i][1] + areasNew[i][2]*areasNew[i][2]);
  //   dASq -= (areas[i][0]*areas[i][0] + areas[i][1]*areas[i][1] + areas[i][2]*areas[i][2]);
  //   printf("dASq: %g\n", dASq);
  // }
