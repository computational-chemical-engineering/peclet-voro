...
// ---------------------------------------------------------------------------
static int testStaticComparison(int n, double Lx, double Ly, double Lz, unsigned int seed,
                                double tol) {
  typedef double real_t;
  ...
  if (maxRelErr > tol) {
    fprintf(stderr, "FAIL: max relative volume error %.2e exceeds tolerance %.2e\n", maxRelErr,
            tol);
    return 1;
  }
  return 0;
...
static int testMovingComparison(int n, double Lx, double Ly, double Lz, unsigned int seed,
  ...
  if (maxRelErr > tol) {
    fprintf(stderr, "FAIL: max relative volume error after move %.2e exceeds tolerance %.2e\n",
            maxRelErr, tol);
    return 1;
  }
  return 0;
