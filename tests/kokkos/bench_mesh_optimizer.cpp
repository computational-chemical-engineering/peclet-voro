 ...
 * packing with a Voronoi tessellation whose seeds sit only in the fluid, then drive the cell
 * volumes to be nearly equal by minimising E = Σ (V_i − V̄)² — and compare how efficiently the
 * different linear-solver / optimiser strategies do it.
 *
 ...
 *       AMG-CG (Jacobi smoother) · AMG-CG (Chebyshev smoother)   [AMG =
 * peclet::core::solver::GraphAMG] reporting iterations + wall time, swept over N to expose the O(N)
 * scaling (AMG flat, the single-level smoothers grow). PART 2 — end-to-end optimisation to a target
 * volume spread: full Newton (Jacobi / colored-GS / AMG preconditioned CG, via meshVolumeOptimize)
 * vs the first-order methods steepest descent and nonlinear (Polak–Ribière) CG. Wall time, outer
 * iterations, final volume spread.
 *
 * Build: part of the voro Kokkos tests (PECLET_VORO_KOKKOS=ON). Run:
 *   ./bench_mesh_optimizer packing.txt [Npart-list...]    e.g.  ./bench_mesh_optimizer packing.txt
 * 4000 16000 40000
 */
#include <algorithm>
#include <chrono>
...
Tess buildTess(const std::vector<Real>& pos, int N, Real L, int sw, const Sdf& sdf,
  ...
  auto res =
      peclet::voro::buildTessellation<Real, false, Sdf>(dpos, dw, N, Larr, sw, N, gd, sdf, true);
  Tess T;
...
// Seed the pore space, then PRUNE seeds whose SDF-clipped cell is degenerate (V<=0) and
// re-tessellate until none remain (a seed in a tight pocket / too close to a wall gets no valid
// cell). This starts the optimiser from a clean nBad==0 state so all methods converge to the same
// minimum — the fair basis for the timing comparison. Over-seeds ~20% to land near the requested
// count after pruning.
template <class Sdf>
...
// Volume variance ⟨(V − ⟨V⟩)²⟩ over the real cells — the equalisation objective; → 0 as cells
// equalise.
...
int main(int argc, char** argv) {
  ...
  Kokkos::initialize(argc, argv);
  {
    ...
    // --------- diagnostic: is the SDF pore case STALLING or CONVERGED? Trace it, and compare to
    // the same seeds tessellated with NO walls (pure Voronoi, which equalises to ~0).
    // --------------------
    bool trace = false;
    for (int i = 2; i < argc; ++i)
    ...
    if (trace) {
      ...
      // Graded reference volume V_ref = clamp(sdf, sLo, sHi)³ (relative; renormalised inside):
      // small cells hugging the walls (inflation layer), growing to a capped bulk size — and it
      // makes the naturally-small wall cells ON-TARGET so the log-barrier no longer fights them.
      ...
      peclet::voro::meshVolumeOptimize<Real, false, SdfSpheres>(
          p2, noW, vset, boxL, Nc, sw, sdf, 200, 1e-9, 400, peclet::voro::Precond::SteepestDescent,
          true, 0.0, 0.7, /*freeEnergy=*/true);
      {
    ...
    for (int N : Ns) {
      ...
      auto runAmg = [&](const char* name, solver::AmgParams ap) {
        ...
        std::printf("%8d | %-22s | %6d | %9.3f | build %.2fs, %d lvls, opC %.2f\n", N, name,
                    R.iters, t, tbuild, M.numLevels(), M.operatorComplexity());
      };
      ...
      run("Jacobi-CG", [&](const std::vector<double>& r, std::vector<double>& z) {
        z.resize(nD);
        for (int i = 0; i < nD; ++i)
          z[i] = std::fabs(H.diag[i]) > 1e-30 ? r[i] / H.diag[i] : r[i];
      });
      ...
      // single-level Chebyshev polynomial preconditioner (no coarse grid: maxLevels=1 +
      // smoother-only)
      {
        solver::AmgParams ap;
        ap.ndofPerNode = 3;
    ...
    std::printf(
        "target: reduce volume variance <(V-<V>)^2>; tol on ‖g‖_inf = 1e-9, max 60 steps\n");
