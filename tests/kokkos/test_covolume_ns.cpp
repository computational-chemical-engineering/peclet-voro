    ...
    // convection) attributes the loss between the two terms.
    {
      ...
      for (int pass = 0; pass < 3; ++pass) {
        ...
        for (int r = 0; r < 3; ++r) {
          ...
          auto m = meshOf(pos, n * n * n, L);
          {  // area-weighted face skewness |c_f - h_A n_f| / d_f (slivers of the degenerate
             // cubic lattice carry no weight)
            auto C = down(m.faceCentroid), Nn = down(m.faceNormal), ha = down(m.faceHa),
                 d = down(m.faceDist), Af = down(m.faceArea);
            double sk = 0, at = 0;
            for (int f = 0; f < m.nInterior; ++f) {
              double t2 = 0;
              for (int c = 0; c < 3; ++c) {
