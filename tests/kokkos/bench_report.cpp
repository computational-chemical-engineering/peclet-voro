...
static void uniform(int N, real_t L, std::mt19937& rng, std::vector<real_t>& x) {
  ...
  x.assign(3 * N, 0);
  for (auto& v : x) v = L * U(rng);
}

// Sum of cell volumes (space-filling: should equal boxVol exactly).
static double sumVol(const Kokkos::View<real_t*, Mem>& vol, int N) {
  double s = 0;
  Kokkos::parallel_reduce("sumvol", Kokkos::RangePolicy<Exec>(0, N),
                          KOKKOS_LAMBDA(int i, double& a) { a += (double)vol(i); }, s);
  return s;
}

// Resident "repair infrastructure" bytes per cell (the topology store + poke4 + per-cell scratch the
// MovingTessellation keeps; excludes the transient grid/facet over-buffers and the position arrays).
static double storeBytesPerCell() {
  ...
  const double scratch = (4 + 3 + 1) * sizeof(int) + 8 * sizeof(int) + sizeof(int);  // masks/wl/extra
...
static int coldSweep(real_t L) {
  ...
  if (const char* e = std::getenv("VORF_NS")) {
    Ns.clear(); std::stringstream ss(e); int n; while (ss >> n) Ns.push_back(n);
  }
  const real_t Larr[3] = {L, L, L};
  std::printf("# cold-build sweep  backend=%s prec=%s MAXP=%d MAXT=%d\n",
              Kokkos::DefaultExecutionSpace::name(), sizeof(real_t) == 4 ? "fp32" : "fp64", CMAXP, CMAXT);
  ...
  for (int N : Ns) {
    ...
    std::vector<real_t> xh; uniform(N, L, rng, xh);
    ...
    double tPlain = 1e30; double volErr = 0;
    for (int r = 0; r < reps; ++r) {
      ...
      Kokkos::fence(); tPlain = std::min(tPlain, secs(a, clk::now()));
      if (r == 0) volErr = std::abs(sumVol(res.view.cellVolume, N) / boxVol - 1.0);
    }
    ...
    for (int r = 0; r < reps; ++r) {
      Kokkos::fence(); auto a = clk::now();
      mt.rebuild(pos);
      Kokkos::fence(); tRepair = std::min(tRepair, secs(a, clk::now()));
    }
...
static int repairSweep(int N, int nSteps, real_t L) {
  ...
  std::printf("# repair sweep  backend=%s prec=%s N=%d nSteps=%d cert=%s gate=%s\n",
              Kokkos::DefaultExecutionSpace::name(), sizeof(real_t) == 4 ? "fp32" : "fp64", N, nSteps,
              bruteCert ? "brute" : "local", noGate ? "off" : "on");
  std::printf("disp,cold_mcells,repair_mcells,speedup,p1pct,p2pct,gatepct,flagpct,vol_err,max_rel_v,missed\n");

  Kokkos::View<real_t*, Mem> x0d("x0", 3 * N), veld("vel", 3 * N), pos("pos", 3 * N);
  Kokkos::View<real_t*, Mem> wd; Kokkos::View<long*, Mem> gd;
  Store oraStore; oraStore.alloc(N);
  std::mt19937 rng(7000);
  std::vector<real_t> x0h; uniform(N, L, rng, x0h);
  std::vector<real_t> velh(3 * N); std::normal_distribution<real_t> Ng(0, 1);
  ...
  auto advance = [&](real_t scale, int step) {
    auto Pd = pos; auto X0 = x0d; auto V = veld; const real_t s = scale * step, Ll = L;
  ...
  for (real_t disp : disps) {
    ...
    for (int s = 1; s <= coldSteps; ++s) {
      ...
      auto r = vor::device::buildTessellation<real_t, false>(pos, wd, N, Larr, 4, N, gd,
          vor::device::NoSdf{}, false);
      Kokkos::fence(); tCold += secs(a, clk::now()); (void)r;
    }
    tCold *= (double)nSteps / coldSteps;  // scale to the per-step×nSteps basis used below
    // repair over the same trajectory
    vor::device::MovingTessellation<real_t, CMAXP, CMAXT> mt;
    mt.localCert = !bruteCert; mt.useGate = !noGate;
    mt.alloc(N, Larr, tol, skin, 4, N);
    advance(scale, 0); mt.rebuild(pos);
    double tRep = 0; long p1 = 0, p2 = 0, gate = 0, flag = 0;
    for (int s = 1; s <= nSteps; ++s) {
      advance(scale, s);
      Kokkos::fence(); auto a = clk::now();
      auto st = mt.step(pos);
      Kokkos::fence(); tRep += secs(a, clk::now());
      p1 += st.pass1; p2 += st.pass2; flag += st.pass1Raw;
    ...
    std::printf("%.4f,%.3f,%.3f,%.2f,%.3f,%.3f,%.1f,%.3f,%.3e,%.3e,%ld\n", (double)disp, coldMc, repMc,
                repMc / coldMc, 100.0 * p1 / ((double)nSteps * N), 100.0 * p2 / ((double)nSteps * N),
                100.0 * gate / nSteps, 100.0 * flag / ((double)nSteps * N), volErr, d.maxVolRelErr,
                d.missedNbr);
    std::fflush(stdout);
  }
  return 0;
