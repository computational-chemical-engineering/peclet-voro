 ...
 * \brief Foundation gates for the `TrackAdj` template axis on ConvexCell (incremental
 * edge-adjacency + Lawson O(nt) local-convexity certificate). Host-side, no parallel_for —
 * exercises the cell methods directly (they are KOKKOS_INLINE_FUNCTION, host-callable).
 *
 * Covers the spec's machine-checkable acceptance:
 *   Change 1  — sizeof: false-mode unchanged, true-mode = false + MAXT*3*sizeof(int)
 * (compile-time). Change 3  — initBox bootstraps a consistent adjacency. Change 2  —
 * rebuildAdjacency() (brute oracle) + checkAdjacencyInvariant() agree on built cells. Change 4  —
 * clip-by-clip in TrackAdj mode keeps checkAdjacencyInvariant() true after EVERY clip, and the
 * incrementally-stitched adj equals rebuildAdjacency() element-for-element. Change 5b —
 * isLocallyConvex == isSelfConsistent in the valid / first-flip (small-δ) regime; the far-poke
 * divergence (brute flags, local does not) is observed at large δ (documents the local
 * certificate's precondition executably).
 */
#include <algorithm>
#include <cstdio>
#include <cstdlib>
...
static_assert(sizeof(ConvexCell<float, 64, 96, false>) == 2828,
              "false-mode (float) sizeof changed");
...
static void buildStepwise(CellT& c, const Nbrs& nb, real_t L, CB&& cb) {
  ...
  for (size_t i = 0; i < nb.id.size(); ++i) {
    ...
    if (!(off < real_t(2) * c.maxVertexRsq()))
      break;  // security radius (same as buildConvexCell)
...
// True iff cell `a`'s incrementally-stitched adj equals a brute rebuild, element-wise on live
// triangles.
...
int main(int argc, char** argv) {
  ...
  int rc = 0;
  {
    ...
    long invFail = 0, bruteFail = 0, matchFail = 0, overflow = 0, cellsChecked = 0,
         clipsChecked = 0;
    for (int s = 0; s < nClouds; ++s) {
      ...
      buildStepwise(c, nb, L, [&](CellT& cc, int clipIdx) {
        ...
        if (!cc.checkAdjacencyInvariant()) {
          localFail = true;
        }
        // Change 4 fuzz: incremental adj == brute rebuild.
        if (!adjMatchesBrute(cc)) {
          localFail = true;
        }
        (void)clipIdx;
        ++clipsChecked;
      ...
      CellT cb = c;
      cb.rebuildAdjacency();
      if (!cb.checkAdjacencyInvariant())
        ++bruteFail;
      if (!adjMatchesBrute(c))
        ++matchFail;
      ++cellsChecked;
    }
    std::printf(
        "[gate B/C/D] clouds=%d cells=%ld clips=%ld | invFail=%ld bruteFail=%ld matchFail=%ld "
        "overflow=%ld\n",
    ...
    // The Lawson local certificate tests a SUBSET of the planes the brute form does (only each
    // edge's edge-opposite plane), so it is a strict subset detector:
    ...
    //       (okB==false && okL==true) — a non-adjacent plane poked a vertex without flipping an
    //       incident edge. This documents the local certificate's small-displacement precondition
    //       executably; it MUST be observed (so the precondition is real, not vacuous).
    ...
    auto sweep = [&](real_t disp, bool isValid) {
      ...
      for (int s = 0; s < 300; ++s) {
        ...
        if (!isValid)
          for (auto& x : Pd)
            x += disp * L * g(r2);  // displace all seeds (topology fixed -> reeval)
        unsigned char poke4[MAXT * 4];
        c.computePoke4(
            poke4);  // build the cert plane set at the BUILD config (then displace + test)
        ...
        if (isValid) {
          ...
          if (okB && !okL)
            ++subsetViolations;  // local flagged what brute did not -> BUG
          if (!okB && okL)
            ++farPoke;  // far-poke the local form misses (expected)
          ++sweepChecked;
        }
        if (!okB && !okL)
          ++localBoth;
    ...
    sweep(0.0, true);
    const long missSmall =
        sweep(0.0005, false);  // operating regime: local must MATCH brute (complete)
    ...
    std::printf("[gate E] validBothTrue=%ld/%ld  subsetViolations=%ld  missSmall@5e-4=%ld\n",
                validBothTrue, validChecked, subsetViolations, missSmall);
    ...
    if (subsetViolations != 0) {
      std::printf(
          "  FAIL: local certificate flagged a cell the brute form calls convex (subset broken)\n");
      rc = 1;
    }
    // With the 4th-face plane the local cert is COMPLETE: in the operating regime it must match the
    // brute flag set (a handful of FP-marginal cells per 300 is tolerated; gross divergence means
    // face4 is broken).
    if (missSmall > 3) {
      std::printf(
          "  FAIL: local cert missed %ld brute flags at small disp (4th-face completeness "
          "broken)\n",
          missSmall);
      rc = 1;
    }
    ...
    if (rc == 0)
      std::printf("ALL TRACKADJ FOUNDATION GATES PASS\n");
  }
  Kokkos::finalize();
  return rc;
}
