...
int main(int argc, char** argv) {
  ...
  int bad = 0;
  {
    ...
    const Real E = co.kineticEnergy(), divMax = co.maxFaceDivergence();
    {  // DIAGNOSTIC: the two copies of each interface flux, and the global mean divergence
      ...
      auto uf = down(co.uf), Af = down(m.faceArea), vol = down(m.cellVolume);
      DV dvv("dvv", m.nCells);
      fv::divergence(m, co.uf, dvv);
      auto dh = down(dvv);
      double sv = 0;
      ...
      MPI_Gatherv(rec.data(), cnt, MPI_DOUBLE, all.data(), cnts.data(), disp.data(), MPI_DOUBLE, 0,
                  MPI_COMM_WORLD);
      if (rank == 0) {
