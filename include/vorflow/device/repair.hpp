 ...
 *
 * `MovingTessellation` keeps the resident topology + geometry of a moving point set and updates it
 * each step WITHOUT a full cold rebuild, falling back to the cold build only when repair cannot
 * close. Per step (`step(pos)`), following dynamic_update_decision_and_plan.md §1/§3:
 *
 *   1. build the per-step grid (buildTessGrid — cheap, memory-bound);
 *   2. re-evaluate every cell on the new positions over its stored topology
 * (ConvexCell::reevalGeometry) and run the convexity/in-sphere certificate, collecting per cell:
 * its own flag (lost-face), the violated-plane partner seeds (the gaining cells of a flip), and the
 * Verlet-skin movers (the insertion trigger the certificate cannot see);
 *   3. **Pass 1** — subset-gather (cold-build kernel over an index list) the set
 *      {flagged ∪ partners ∪ skin-movers}: those cells get exact, freshly-clipped topology +
 * volume;
 *   4. **Pass 2** — subset-gather the NEW face-neighbours of the Pass-1 cells that were not
 * themselves rebuilt (chiefly a far-mover's new neighbours, which never flag — §1c);
...
struct MovingTessellation {
  ...
  // repair's hot kernel). Only the cell-BUILDING paths (the gather's CellBuilder, rebuildAdjAll,
  // surgicalRepair) need `adj` to derive poke4, so they use the TrackAdj=true BuildCell.
  ...
  double churnThresh =
      0.55;  ///< flagged-fraction above which a full rebuild is cheaper than repairing
             ///< (per-backend, set in alloc — GPU rebuild is cheap so it gives up sooner).
  double dilateMaxChurn =
      0.15;  ///< only consider dilation when GLOBAL churn is low (a real local cluster,
             ///< not a uniformly-moderate field where every neighbourhood looks "dense").
  int clusterNbhd = 18;  ///< a cell whose 27-grid-cell neighbourhood has > this many flagged seeds
                         ///< is "dense" (≈ a region far above the field average) and gets dilated.
  // ---- Phase 4: single-face surgical repair (no grid gather; gated, default OFF — see note) ----
  // NEGATIVE RESULT (measured): on this engine surgical re-clip is BOTH slower and not robustly
  // exact, so it stays off. (1) Slower: the cold gather clips closest-first with the
  // security-radius early-out (the cell tightens fast, few live triangles per clip), whereas
  // re-clipping the stored+partner candidates unsorted keeps the cell large and costs more O(#tri)
  // horizon scans — the gather wins. (2) Not exact: a surgically-rebuilt cell that GAINS a
  // neighbour absent from its candidate set stays convex, so the certificate can't see the gain
  // (§1c) and the verify loop can't catch it; the small per-step error then COMPOUNDS in the
  // resident store (maxRelV grew to ~0.5 over a sweep). Making it exact needs the maintained
  // 1-/2-ring candidate set (the ConnectivityArena the plan parks). Kept as the implemented
  // experiment behind this flag; the production path is the Phase-3 gated two-pass gather.
  bool surgical =
      false;  ///< repair flip cells by re-clipping known candidates instead of gathering
  ...
  bool localCert =
      true;  ///< use ConvexCell::isLocallyConvexPartners (read each triangle's 4 precomputed
             ///< `poke4` planes — 3 edge-opposite + 4th-face — O(nt)) instead of the brute
             ///< O(nt·np) form. The gather derives poke4 from the clip-maintained edge
             ///< adjacency (computePoke4, no findSharing) into store.poke4, so it is fused
             ///< into the build with no separate maintenance pass, and the cert reads it
             ///< directly each step. With the 4th-face plane the local cert is COMPLETE
             ///< (matches the brute flag set — bench GATE 2), so there is no fallback gate.
  bool adjFresh =
      false;  ///< store.poke4 is valid for the whole current topology. Gathers maintain it
  ...
  /// @param nProc_  cells to maintain (default n = single-domain). For the distributed driver pass
  /// the
  ...
  void alloc(int n, const Real Lbox[3], Real tol_, Real skin_, int sw_ = 4, int densityCount_ = -1,
    ...
    sw = sw_;
    densityCount = densityCount_ < 0 ? n : densityCount_;
    tol = tol_;
    skin = skin_;
    for (int k = 0; k < 3; ++k)
      L[k] = Lbox[k];
    store.alloc(n);
    store.allocPoke4();  // resident local-cert plane set (computed by the gather, read each step by
                         // the cert)
    using Kokkos::view_alloc;
    using Kokkos::WithoutInitializing;
    vol = Kokkos::View<Real*, Mem>("mt.vol", n);
    xRef = Kokkos::View<Real*, Mem>(view_alloc(std::string("mt.xRef"), WithoutInitializing),
                                    (size_t)n * 3);
    ...
    // Per-backend gate threshold: rebuild is cheap + clip-bound on GPU (repair loses past ~50%
    // flagged), expensive on the CPU paths (repair pays out to ~85%). Hand-set from the
    // cross-device sweep; the plan's Phase-3 tuning would fit these from logged (signals vs
  ...
  /// @param eagerAdj  populate store.poke4 now (default; the cheap initial/explicit rebuild → next
  /// step is
  ///                  local). A GATE-rebuild passes false: in a high-churn regime we keep
  ///                  rebuilding, so paying rebuildAdjAll every step is wasted — fall back to the
  ...
  void rebuild(const Kokkos::View<Real*, Mem>& pos, bool eagerAdj = true) {
    ...
    if (eagerAdj)
      maybeBuildAdj(pos);  // the whole topology changed -> repopulate store.poke4 (unless gating)
  }
  ...
  int compact(const Kokkos::View<int*, Mem>& m, const Kokkos::View<int*, Mem>& wl) {
    ...
    auto c = counter;
    Kokkos::parallel_for(
        "mt.compact", Kokkos::RangePolicy<Exec>(0, nProc), KOKKOS_LAMBDA(int i) {
          if (M(i))
            W(Kokkos::atomic_fetch_add(&c(), 1)) = i;
        });
    int h = 0;
  ...
  ///   `outMask(i)=1`  if the cell is inconsistent (lost-face) OR is a violated-plane partner of
  ///   some
  ...
  /// `useSkin` gates the mover trigger (off during verify passes — movers are a once-per-step
  /// trigger already folded into Pass 1, and the verify only needs to re-close residual flips).
  template <bool Local>
  void certify(const Kokkos::View<Real*, Mem>& pos, const Kokkos::View<int*, Mem>& outMask,
    ...
    if (useSkin)
      Kokkos::deep_copy(outMover, 0);
    // Phase-4: in the Pass-1 certify, also record partner-discovered new neighbours for the
    // surgical path. The gaining cells of a flip are the violated-plane partners; each pair gains
    // the other, so scatter every other partner of a flagging cell onto each partner's
    // extra-candidate list (no dedup — duplicates are harmless no-op clips; overflow past kExtraCap
    // just truncates and the cell falls back to a full gather at verify). Only in Pass 1 (useSkin),
    // and only when surgical is enabled.
    const bool doExtra = surgical && useSkin;
    if (doExtra)
      Kokkos::deep_copy(extraCnt, 0);
    auto EN = extraNbr;
    auto EC = extraCnt;
    ...
    Kokkos::parallel_for(
        "mt.certify", Kokkos::RangePolicy<Exec>(0, nProc), KOKKOS_LAMBDA(int i) {
          ...
          int nP = 0;
          bool ok;
          // Force-capture every lambda capture used in the branches OUTSIDE the constexpr-if (nvcc:
          // an extended __host__ __device__ lambda cannot FIRST-capture a variable inside a
          // constexpr-if).
          const unsigned char* pk4 = &Pk4((size_t)i * MAXT * 4);  // poke4 is always allocated
          const Real tl = tolL;
          if constexpr (Local)
            ok = c.isLocallyConvexPartners(pk4, tl, partners, 32, nP);
          else {
            (void)pk4;
            ok = c.isSelfConsistentPartners(tl, partners, 32, nP);
          }
          bool mover = false;
          if (skinOn) {
          ...
          if (doExtra) {  // each pair of gaining cells gains the other -> seed the surgical
                          // candidate lists
            for (int q = 0; q < nP; ++q) {
  ...
  /// Scoped re-certify: reeval + certificate over ONLY the `n` cells in `list` (refresh their vol),
  /// set outMask(i)=1 for any inconsistent + OR-mark their partners. The verify uses this instead
  /// of a full re-certify: after the gathers, every NON-gathered cell's reeval is bit-identical to
  /// the Pass-1 certify (its store + its neighbours' positions are unchanged), and every gained
  /// edge has BOTH endpoints gathered (a flip's partner endpoints in Pass 1, a mover's new
  /// neighbours in Pass 2), so only the gathered cells can differ — re-checking just them is
  /// provably equivalent to a full re-certify at a fraction of the cost (this is what lets the
  /// small-displacement speedup rise instead of being pinned by a second full-N reeval).
  template <bool Local>
  void certifyList(const Kokkos::View<Real*, Mem>& pos, const Kokkos::View<int*, Mem>& list, int n,
    ...
    Kokkos::parallel_for(
        "mt.certifyList", Kokkos::RangePolicy<Exec>(0, n), KOKKOS_LAMBDA(int s) {
          ...
          int nP = 0;
          bool ok;
          const unsigned char* pk4 =
              &Pk4((size_t)i * MAXT * 4);  // force-capture poke4 + tol OUTSIDE the
  ...
  /// buildTessellation (which does not emit adjacency); the frequent gathers maintain adj
  /// incrementally. The result is element-for-element identical to the gather's incremental adj
  /// (validated foundation gate), so the two adj sources mix cleanly across the store.
  ...
  /// Lazily repopulate store.poke4 on a low-churn step so later steps use the cheap local
  /// certificate. A gate-rebuild clears adjFresh; gating steps return before this runs, so a
  /// sustained high-churn regime keeps falling back to the brute certificate (no adj overhead), and
  /// a low-churn streak pays the full rebuildAdjAll exactly once (then gathers maintain adj
  /// incrementally).
  ...
  /// Gather the `n` cells listed in `wl[0..n)` off `grid` (updating store + vol in place), mark
  /// them rebuilt, and RESET their Verlet reference to the current position (their neighbourhood is
  /// now fresh, so the skin/2 mover test for these cells restarts from here — proper per-cell
  /// Verlet). Reuses the per-step grid.
  void gatherSet(const TessGrid<Real>& grid, const Kokkos::View<int*, Mem>& wl, int n,
    ...
    auto XR = xRef;
    Kokkos::parallel_for(
        "mt.markRebuilt", Kokkos::RangePolicy<Exec>(0, n), KOKKOS_LAMBDA(int s) {
          const int i = W(s);
          rb(i) = 1;
          XR(3 * i) = P(3 * i);
          XR(3 * i + 1) = P(3 * i + 1);
  ...
  void collectNewNbrs(const Kokkos::View<int*, Mem>& wl, int n,
                      const Kokkos::View<int*, Mem>& outMask) {
    Kokkos::deep_copy(outMask, 0);
    if (n <= 0)
      return;
    ...
    Kokkos::parallel_for(
        "mt.collectNbrs", Kokkos::RangePolicy<Exec>(0, n), KOKKOS_LAMBDA(int s) {
          ...
          for (int k = 6; k < np; ++k) {
            ...
            if (j < 0 || j >= nP_ || rb(j))
              continue;   // only expand neighbours we MAINTAIN (owned)
            int cnt = 0;  // face = ≥3 incident live triangles
            for (int t = 0; t < nt; ++t) {
              const unsigned w = st.tri((size_t)i * MAXT + t);
              if (!((w >> 24) & 1u))
                continue;
              const int a = (int)(w & 0xffu), b = (int)((w >> 8) & 0xffu),
                        cc = (int)((w >> 16) & 0xffu);
              if (a == k || b == k || cc == k)
                ++cnt;
            }
            if (cnt >= 3)
              Kokkos::atomic_exchange(&M(j), 1);
          }
        });
  }

  /// Phase-4 single-face surgical repair: rebuild each flip cell in wl[0..n) by re-clipping the box
  /// with its KNOWN candidate set — stored neighbours ∪ partner-discovered extras (extraNbr) — at
  /// the current positions, with NO grid gather (the clip is order-independent and only commits
  /// planes that cut, so stale/lost-face neighbours are harmless no-ops). Updates store + vol,
  /// marks rebuilt, resets the Verlet reference. A cell whose candidate set is incomplete comes out
  /// wrong and is caught + re-gathered by the verify loop (the exact fallback) — so surgical is
  /// safe by construction. Used only for FLIP cells; far-mover insertions keep the full gather
  /// (their new neighbours aren't in any candidate list).
  void surgicalRepair(const Kokkos::View<int*, Mem>& wl, int n,
                      const Kokkos::View<Real*, Mem>& pos) {
    if (n <= 0)
      return;
    auto W = wl;
    ...
    const Real Lxh = Real(0.5) * Lx, Lyh = Real(0.5) * Ly, Lzh = Real(0.5) * Lz;
    Kokkos::parallel_for(
        "mt.surgical", Kokkos::RangePolicy<Exec>(0, n), KOKKOS_LAMBDA(int s) {
          const int i = W(s);
          BuildCell c;
          c.initBox(Lx, Ly, Lz);
          ...
          if (!c.overflow) {
            ...
            Vv(i) = c.volumePerVertex();
            c.computePoke4(
                &Pk4((size_t)i * MAXT * 4));  // refresh the cert plane set for this re-clipped cell
          }
          rb(i) = 1;
          XR(3 * i) = sx;
          XR(3 * i + 1) = sy;
  ...
  /// Dense-cluster dilation (Phase 3): bin the flagged cells into the linear grid, then add every
  /// owned cell whose 27-grid-cell neighbourhood holds > clusterNbhd flagged seeds to the Pass-1
  /// mask. This over-covers a local cascade in ONE regional gather instead of paying a
  /// kernel-launch per verify iteration. Over-coverage is always safe (a gathered cell is just
  /// re-clipped correctly). Returns true if it added any cell. Uses the linear grid index
  /// (independent of the grid's Morton ordering).
  bool dilate(const TessGrid<Real>& grid, const Kokkos::View<Real*, Mem>& pos) {
    ...
    if ((int)cellFlag.extent(0) < ncell)
      cellFlag = Kokkos::View<int*, Mem>(
          Kokkos::view_alloc(std::string("mt.cellFlag"), Kokkos::WithoutInitializing), ncell);
    ...
    auto CF = cellFlag;
    Kokkos::parallel_for(
        "mt.dilate.count", Kokkos::RangePolicy<Exec>(0, nProc), KOKKOS_LAMBDA(int i) {
          if (!M(i))
            return;
          ...
          for (int dz = -1; dz <= 1; ++dz)
            for (int dy = -1; dy <= 1; ++dy)
              for (int dx = -1; dx <= 1; ++dx) {
                const int nx = (gx + dx + dimx) % dimx, ny = (gy + dy + dimy) % dimy,
                          nz = (gz + dz + dimz) % dimz;
  ...
  /// Launch-time dispatch of the certificate kernel (Change 7): the gate decides local vs brute,
  /// then launches the corresponding COMPILE-TIME specialization — there is no runtime cert branch
  /// inside the kernel. `useLocalNow` is the only place this is decided per step.
  void certifyDispatch(const Kokkos::View<Real*, Mem>& pos, const Kokkos::View<int*, Mem>& outMask,
                       const Kokkos::View<int*, Mem>& outMover, bool useSkin) {
    if (useLocalNow)
      certify<true>(pos, outMask, outMover, useSkin);
    else
      certify<false>(pos, outMask, outMover, useSkin);
  }
  void certifyListDispatch(const Kokkos::View<Real*, Mem>& pos, const Kokkos::View<int*, Mem>& list,
                           int n, const Kokkos::View<int*, Mem>& outMask) {
    if (useLocalNow)
      certifyList<true>(pos, list, n, outMask);
    else
      certifyList<false>(pos, list, n, outMask);
  }
  ...
  RepairStats step(const Kokkos::View<Real*, Mem>& pos) {
    ...
    // subset), so no per-step displacement gate or brute fallback is needed — large moves and
    // teleports are handled by the same detect→gather→verify path. (adj/face4 are invalidated only
    // by a cold rebuild.)
    useLocalNow = localCert && adjFresh;

    // Pass 1: detect (flagged ∪ partners ∪ skin-movers). flagged-commons + their violated-plane
    // partners (the gaining cells) fully cover an isolated flip — no Pass 2 for flips.
    ...
    // always flags some cell, so n1==0 ⇒ the stored topology is still Voronoi on the new positions
    // — the re-eval already refreshed every volume, so publish with NO gather and NO verify pass.
    // This is the "almost no cells invalidated" regime: the per-step cost collapses to one re-eval
    // + the grid.
    if (n1 == 0) {
      maybeBuildAdj(pos);
      return s;
    ...
    if (useGate && useDilation && n1 > 0 && (double)n1 / nProc < dilateMaxChurn &&
        dilate(grid, pos)) {
      n1 = compact(mask, wl1);
    ...
    // Pass 1 gather. Phase 4: FLIP cells (flagged non-movers) are repaired SURGICALLY (re-clip
    ...
    if (surgical) {
      ...
      auto M2 = mask2;
      Kokkos::parallel_for(
          "mt.surgMask", Kokkos::RangePolicy<Exec>(0, nProc),
          KOKKOS_LAMBDA(int i) { M2(i) = (Msk(i) && !Mv(i)) ? 1 : 0; });
      int nS =
          compact(mask2, wl1);  // wl1's Pass-1 list no longer needed -> reuse for the flip list
      s.surgical = nS;
    ...
    // Pass 2: ONLY the new face-neighbours of the skin-MOVERS (the insertion side §1c — those
    // neighbours never flag and are never partners). Expanding every Pass-1 cell would re-clip ~all
    // neighbours of every flip cluster for nothing; movers are the only cells whose rebuild reveals
    // an unflagged gainer.
    ...
    maybeBuildAdj(
        pos);  // first low-churn step after a rebuild: populate adj so verify + next steps go local

    // Verify (scoped): re-certify only the GATHERED cells (every non-gathered cell is provably
    // unchanged — see certifyList). Clean ⇒ publish; else gather the still-flagged ∪ partners
    // (residual coupled flips / tol-degenerate cells) and re-verify, bounded.
    for (int vp = 0; vp < verifyCap + 1; ++vp) {
      const int ng = compact(rebuilt, wl1);  // all cells gathered so far this step
      // Verify with the step's certificate: a far/teleport step already forced useLocalNow=false,
      // so its verify is the COMPLETE brute form (catches far-pokes the gather left on a gathered
      // cell). A small- displacement local step uses the cheap local verify over the
      // freshly-gathered (locally convex) set.
      ...
      if (nv == 0)
        return s;  // clean — done
      if (vp == verifyCap)
        break;  // out of budget — fall back
      s.extra += nv;
      gatherSet(grid, wl2, nv, pos);
    }

    // Still dirty after the verify budget: cold rebuild (always exact). Rare; the data point that
    // says "for this step, rebuild was cheaper" — the adaptive gate (Phase 3) will pre-empt it up
    // front.
    rebuild(pos);
    s.fellBack = true;
    return s;
