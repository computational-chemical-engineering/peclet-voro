 ...
 *
 * Given a TessGrid built once for the current positions (vorflow/device/tess_grid.hpp), `subsetGather`
 * runs the EXACT same `CellBuilder::buildCell` the cold build runs — same worklist gather, same
 * ConvexCell clip — but only over the grid-sorted slots of the requested ORIGINAL seed indices
 * (mapped through grid.slotOf). It writes the resident topology store (np/nt/pnbr/tri) and the cell
 * volume at the original index, so a subset cell is byte-for-byte the cell buildTessellation would
 * produce for that index off the same grid (the gather is per-cell independent; restricting the
 * launch set changes nothing about any individual cell's computation).
 *
...
SubsetGatherResult<Real> subsetGather(const TessGrid<Real>& grid,
                                      ...
                                      const Kokkos::View<unsigned char*, tpx::MemSpace>& outPoke4 = {},
  ...
  // the topology store is the published Phase-1 output, this buffer is just buildCell's scratch.
  ...
  Kokkos::View<int*, MemSpace> oNbr(view_alloc(std::string("subset.oNbr"), WithoutInitializing), facetCap);
  Kokkos::View<Real*, MemSpace> oArea(view_alloc(std::string("subset.oArea"), WithoutInitializing), facetCap * 3);
  Kokkos::View<Real*, MemSpace> oDV(view_alloc(std::string("subset.oDV"), WithoutInitializing), facetCap * 3);
  Kokkos::View<Real*, MemSpace> oConn(view_alloc(std::string("subset.oConn"), WithoutInitializing), facetCap * 3);
  ...
  Builder op{
      ...
      outNp, outNt, outPnbr, outTri, outPoke4, noCand, noCandCnt,
  ...
  Kokkos::parallel_for(
      "subset.build", Kokkos::RangePolicy<Exec>(0, nSubset), KOKKOS_LAMBDA(const int s) {
        ...
        op.buildCell(slotOf(i));  // grid-sorted slot of that seed; writes outputs at binned(slot)==i
      });
  Kokkos::fence();
  return res;
}
