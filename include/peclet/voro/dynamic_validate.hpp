 ...
 * \brief Part-II Phase-0 validators: per-step invariants + oracle diff for the moving-point
 * updater.
 *
 * You cannot trust any incremental update result without (a) cheap per-step invariants that a
 * corrupt update violates, and (b) a periodic comparison against the cold-build oracle (the same
 * buildTessellation that is also the rebuild fallback). This header provides both, on device, in
 * one Kokkos path. They are diagnostic — call them every step in debug / every k steps in
 * production; the production hot loop pays nothing unless it opts in.
 *
 ...
 *   - maxAreaAsym : max over interior facets of |A(g) + A(recip(g))| / |A(g)|  (A_ij = −A_ji
 * exactly: the momentum-conservation certificate — any pairwise force ∝ A then sums to zero).
 ...
 *   - nNonRecip   : interior facets (neighbour in [0,N)) with no reciprocal — a topology-symmetry
 * leak.
 *
...
Invariants checkInvariants(const TessellationView<Real>& view, const AuxMaps<Real>& aux,
  ...
  Kokkos::parallel_reduce(
      ...
      KOKKOS_LAMBDA(const int g, double& ax, double& ay, double& az, double& fx, double& fy,
                    double& fz) {
        ax += (double)view.area(g, 0);
        ay += (double)view.area(g, 1);
        az += (double)view.area(g, 2);
        fx += (double)view.connect(g, 0);
        fy += (double)view.connect(g, 1);
        fz += (double)view.connect(g, 2);
      },
...
struct OracleDiff {
  ...
  double changedNbrFrac =
      0;               ///< fraction of cells whose stored neighbour set differs from the oracle
...
/// plane k counts only if ≥3 live triangles are incident — the raw `pnbr` also keeps non-face
/// cutting planes, which are not neighbours). changedNbrFrac = cells whose face-neighbour SET
/// differs (either direction); missedNbr = cells missing a TRUE neighbour (genuine staleness — what
...
void compareNeighbours(const Kokkos::View<int*, peclet::core::MemSpace>& storeNp,
  ...
  Kokkos::parallel_reduce(
      ...
      KOKKOS_LAMBDA(const int i, long& chg, long& mis) {
        ...
        // every true (oracle) neighbour must be a store FACE neighbour (≥3 incident live
        // triangles).
        for (int g = oracle.facetBegin(i); g < oracle.facetEnd(i); ++g) {
          ...
          for (int k = 6; k < snp && !inStore; ++k) {
            if (storePnbr((size_t)i * MAXP + k) != j)
              continue;
            int cnt = 0;
            for (int t = 0; t < snt; ++t) {
              ...
              if (!((w >> 24) & 1u))
                continue;
              const int t0 = (int)(w & 0xffu), t1 = (int)((w >> 8) & 0xffu),
                        t2 = (int)((w >> 16) & 0xffu);
              if (t0 == k || t1 == k || t2 == k)
                ++cnt;
            }
