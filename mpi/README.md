...
- `validate_voronoi_dynamics.py` — 6 steps of distributed compressible-Euler **dynamics** match serial
  to machine precision (~4e-15). NB: the dynamics needs a **2-ring** halo (`rcut≈2×` the tessellation
  depth) because the force uses neighbours' cell pressures; see the doc.
