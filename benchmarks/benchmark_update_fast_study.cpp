 ...
 *
 * Output:
 *  CSV rows, one per timestep, with repeated final-comparison metrics so a
 *  report script can aggregate results across multiple dt runs.
 */
...
int main(int argc, char **argv) {
  ...
  if (num_particles <= 0 || num_steps <= 0 || dt <= 0.0 || velocity_sigma < 0.0) {
    std::fprintf(stderr,
                 "Usage: %s [N=10000] [steps=6] [dt=1e-3] [velocity_sigma=1.0] [pos_seed] [vel_seed]\n",
                 argv[0]);
    return 1;
  }
  ...
  const int initial_non_convex_at_build = CountNonConvexCells(complex_incremental, pos, box);
  ...
  ComputeVolumeDiffStatsByCellId(complex_incremental, complex_clean, num_particles,
                                 &final_max_abs_volume_diff, &final_max_rel_volume_diff,
                                 &final_total_volume_update, &final_total_volume_rebuild);

  std::printf("# benchmark_update_fast_study\n");
  std::printf("# Compiled: %s %s\n", __DATE__, __TIME__);
  std::printf("# N=%d steps=%d dt=%.8g velocity_sigma=%.6f pos_seed=%llu vel_seed=%llu\n",
              num_particles, num_steps, dt, velocity_sigma,
              static_cast<unsigned long long>(pos_seed),
              static_cast<unsigned long long>(vel_seed));
  std::printf(
      "N,steps,dt,velocity_sigma,pos_seed,vel_seed,initial_non_convex_at_build,step,update_ms,"
  ...
  for (const StepStats &s : step_stats) {
    std::printf(
        "%d,%d,%.16e,%.16e,%llu,%llu,%d,%d,%.16e,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%.16e,%d,%d,%.16e,%.16e,%.16e,%.16e\n",
        num_particles, num_steps, dt, velocity_sigma,
        static_cast<unsigned long long>(pos_seed), static_cast<unsigned long long>(vel_seed),
        initial_non_convex_at_build, s.step, s.update_ms, s.non_convex_before, s.non_convex_after,
        s.rebuild_candidates, s.local_rebuild_cells, s.empty_after_local_rebuild,
        s.full_rebuild_cells, s.repair_iterations, s.repair_proposals_total,
        s.repair_target_groups_total, s.repair_cells_changed_total, s.repair_direct_attempts,
        ...
        s.max_convex_violation_after, final_signature_mismatch_cells, final_non_reciprocal_pairs,
        final_max_abs_volume_diff, final_max_rel_volume_diff, final_total_volume_update,
